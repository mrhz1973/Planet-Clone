...
Tile :  http://localhost:5000/tiles/{z}/{x}/{y}.png
...
#   transparent: false = strato base, true = overlay sovrapponibile
...
TILE_PARAMS = {
    "transparent": os.environ.get("NAV_TRANSPARENT", "false"),
    "ugc": os.environ.get("NAV_UGC", "false"),
    "layer": os.environ.get("NAV_LAYER", "0"),
...
def status():
    ...
    return jsonify({
        ...
        "layer": TILE_PARAMS["layer"],
        "transparent": TILE_PARAMS["transparent"],
...
def proxy_tile(z, x, y):
    for attempt in range(2):
        ...
        params = dict(TILE_PARAMS, config=config)
        try:
            resp = _session.get(url, headers=headers, params=params, timeout=10)
        except requests.RequestException as exc:
            print(f"[proxy] Errore rete tile {z}/{x}/{y}: {exc}")
            break
        ...
        if resp.status_code == 401 and attempt == 0:
            print(f"[proxy] 401 su {z}/{x}/{y}: rinnovo token...")
            continue

        print(f"[proxy] tile {z}/{x}/{y} -> {resp.status_code} ({ctype})")
        break
...
if __name__ == "__main__":
    ...
    print("Tile : http://localhost:5000/tiles/{z}/{x}/{y}.png")
